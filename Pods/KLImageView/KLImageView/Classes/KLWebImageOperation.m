...
static BOOL KLCGImageLastPixelFilled(CGImageRef image) {
    if (!image) return NO;
    size_t width = CGImageGetWidth(image);
    size_t height = CGImageGetHeight(image);
    if (width == 0 || height == 0) return NO;
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, KLCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface _KLWebImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _KLWebImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_KLWebImageWeakProxy alloc] initWithTarget:target];
}
...
@interface KLWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) KLImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) KLWebImageProgressBlock progress;
@property (nonatomic, copy) KLWebImageTransformBlock transform;
@property (nonatomic, copy) KLWebImageCompletionBlock completion;
@end


@implementation KLWebImageOperation
...
+ (void)_networkThreadMain:(id)object {
    @autoreleasepool {
        [[NSThread currentThread] setName:@"com.kalan.webimage.request"];
    ...
    dispatch_once(&onceToken, ^{
        ...
        if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
            for (NSUInteger i = 0; i < queueCount; i++) {
                ...
                queues[i] = dispatch_queue_create("com.kalan.image.decode", attr);
            }
        } else {
            for (NSUInteger i = 0; i < queueCount; i++) {
                queues[i] = dispatch_queue_create("com.kalan.image.decode", DISPATCH_QUEUE_SERIAL);
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"KLWebImageOperation init error" reason:@"KLWebImageOperation must be initialized with a request. Use the designated initializer to init." userInfo:nil];
...
- (void)dealloc {
    ...
    if (_taskID != UIBackgroundTaskInvalid) {
        [_KLSharedApplication() endBackgroundTask:_taskID];
        _taskID = UIBackgroundTaskInvalid;
    }
    if ([self isExecuting]) {
        self.cancelled = YES;
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & KLWebImageOptionShowNetworkActivity)) {
                [KLWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, KLWebImageFromNone, KLWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & KLWebImageOptionUseNSURLCache) &&
            !(_options & KLWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:KLImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, KLWebImageFromMemoryCache, KLWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & KLWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:KLImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:KLImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & KLWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, KLWebImageFromNone, KLWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[_KLWebImageWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & KLWebImageOptionShowNetworkActivity)) {
                [KLWebImageManager incrementNetworkActivityCount];
            }
        }
        [_lock unlock];
    }
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, KLWebImageFromNone, KLWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, KLWebImageFromDiskCache, KLWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & KLWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([KLWebImageOperation _imageQueue], ^{
                        KLImageCacheType cacheType = (_options & KLWebImageOptionIgnoreDiskCache) ? KLImageCacheTypeMemory : KLImageCacheTypeAll;
            ...
            NSError *error = nil;
            if (!image) {
                error = [NSError errorWithDomain:@"com.kalan.image" code:-1 userInfo:@{ NSLocalizedDescriptionKey : @"Web image decode fail." }];
                if (_options & KLWebImageOptionIgnoreFailedURL) {
            ...
            if (_completion) _completion(image, _request.URL, KLWebImageFromRemote, KLWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & KLWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & KLWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & KLWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & KLWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[KLImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == KLImageTypeUnknown ||
            _progressiveDecoder.type == KLImageTypeWebP ||
            _progressiveDecoder.type == KLImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != KLImageTypeJPEG &&
                _progressiveDecoder.type != KLImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            KLImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, KLWebImageFromRemote, KLWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == KLImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (!KLCGImageLastPixelFilled(image.CGImage)) return;
            ...
            image = [image kl_imageByBlurRadius:radius tintColor:nil tintMode:0 saturation:1 maskImage:nil];
            
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, KLWebImageFromRemote, KLWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & KLWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & KLWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[KLImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image kl_imageByDecoded];
                    if ([((KLImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    KLImageDecoder *decoder = [KLImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                KLImageType imageType = KLImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case KLImageTypeJPEG:
                    case KLImageTypeGIF:
                    case KLImageTypePNG:
                    case KLImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == KLImageTypeGIF ||
                                imageType == KLImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & KLWebImageOptionShowNetworkActivity)) {
                [KLWebImageManager decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, KLWebImageFromNone, KLWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & KLWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & KLWebImageOptionAllowBackgroundTask) && _KLSharedApplication()) {
                    __weak __typeof__ (self) _self = self;
                    if (_taskID == UIBackgroundTaskInvalid) {
                        _taskID = [_KLSharedApplication() beginBackgroundTaskWithExpirationHandler:^{
