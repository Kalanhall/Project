...
typedef NS_ENUM(NSUInteger, KLWebImageFromType) {
    ...
    KLWebImageFromMemoryCacheFast,
    
    /// Fetched from memory cache.
    KLWebImageFromMemoryCache,
    
    /// Fetched from disk cache.
    KLWebImageFromDiskCache,
    
    /// Fetched from remote (web or file path).
    KLWebImageFromRemote,
};

/// Indicated image fetch complete stage.
typedef NS_ENUM(NSInteger, KLWebImageStage) {
    
    /// Incomplete, progressive image.
    KLWebImageStageProgress  = -1,
    
    /// Cancelled.
    KLWebImageStageCancelled = 0,
    
    /// Finished (succeed or failed).
    KLWebImageStageFinished  = 1,
};
 ...
 */
typedef void(^KLWebImageProgressBlock)(NSInteger receivedSize, NSInteger expectedSize);

/**
 The block invoked before remote image fetch finished to do additional image process.
 
 @discussion This block will be invoked before `KLWebImageCompletionBlock` to give
 ...
 @example You can clip the image, blur it and add rounded corners with these code:
    ^(UIImage *image, NSURL *url) {
        ...
        image = [image kl_imageByResizeToSize:CGSizeMake(100, 100) contentMode:UIViewContentModeScaleAspectFill];
        image = [image kl_imageByBlurRadius:20 tintColor:nil tintMode:kCGBlendModeNormal saturation:1.2 maskImage:nil];
        image = [image kl_imageByRoundCornerRadius:5];
        return image;
    }
 ...
 */
typedef UIImage * _Nullable (^KLWebImageTransformBlock)(UIImage *image, NSURL *url);
 ...
 */
typedef void (^KLWebImageCompletionBlock)(UIImage * _Nullable image,
                                          NSURL *url,
                                          KLWebImageFromType from,
                                          KLWebImageStage stage,
 ...
 */
@interface KLWebImageManager : NSObject

/**
 Returns global KLWebImageManager instance.
 
 @return KLWebImageManager shared instance.
 */
 ...
 */
- (instancetype)initWithCache:(nullable KLImageCache *)cache
 ...
 */
- (nullable KLWebImageOperation *)requestImageWithURL:(NSURL *)url
 ...
 */
@property (nullable, nonatomic, strong) KLImageCache *cache;
 ...
 */
@property (nullable, nonatomic, copy) KLWebImageTransformBlock sharedTransformBlock;
...
/**
 ...
 @param url A specified URL
 @return Cache key used in KLImageCache.
 */
