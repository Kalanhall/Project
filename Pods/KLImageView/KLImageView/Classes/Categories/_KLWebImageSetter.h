//
//  _KLWebImageSetter.h
//  KLWebImage <https://github.com/kalanhall/KLWebImage>
//
//  Created by kalan on 19/7/15.
//  Copyright (c) 2019 kalan.

#import <UIKit/UIKit.h>
#import <pthread.h>

#if __has_include(<KLWebImage/KLWebImage.h>)
#import <KLWebImage/KLWebImageManager.h>
#else
#import "KLWebImageManager.h"
#endif

NS_ASSUME_NONNULL_BEGIN

/**
 Submits a block for execution on a main queue and waits until the block completes.
 */
static inline void _kl_dispatch_sync_on_main_queue(void (^block)()) {
    if (pthread_main_np()) {
        block();
    } else {
        dispatch_sync(dispatch_get_main_queue(), block);
    }
}

extern NSString *const _KLWebImageFadeAnimationKey;
extern const NSTimeInterval _KLWebImageFadeTime;
extern const NSTimeInterval _KLWebImageProgressiveFadeTime;

/**
 Private class used by web image categories.
 Typically, you should not use this class directly.
 */
@interface _KLWebImageSetter : NSObject
/// Current image url.
@property (nullable, nonatomic, readonly) NSURL *imageURL;
/// Current sentinel.
@property (nonatomic, readonly) int32_t sentinel;

/// Create new operation for web image and return a sentinel value.
- (int32_t)setOperationWithSentinel:(int32_t)sentinel
                                url:(nullable NSURL *)imageURL
                            options:(KLWebImageOptions)options
                            manager:(KLWebImageManager *)manager
                           progress:(nullable KLWebImageProgressBlock)progress
                          transform:(nullable KLWebImageTransformBlock)transform
                         completion:(nullable KLWebImageCompletionBlock)completion;

/// Cancel and return a sentinel value. The imageURL will be set to nil.
- (int32_t)cancel;

/// Cancel and return a sentinel value. The imageURL will be set to new value.
- (int32_t)cancelWithNewURL:(nullable NSURL *)imageURL;

/// A queue to set operation.
+ (dispatch_queue_t)setterQueue;

@end

NS_ASSUME_NONNULL_END
