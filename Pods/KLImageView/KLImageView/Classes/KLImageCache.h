...
@class KLMemoryCache, KLDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, KLImageCacheType) {
    /// No value.
    KLImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    KLImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    KLImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    KLImageCacheTypeAll    = KLImageCacheTypeMemory | KLImageCacheTypeDisk,
};


/**
 KLImageCache is a cache that stores UIImage and image data based on memory cache and disk cache.
 ...
 */
@interface KLImageCache : NSObject
...
/**
 ...
 @discussion When fetch image from disk cache, it will use 'KLImage' to decode 
...
/**
 ...
 @return  The singleton KLImageCache instance.
 */
...
/**
 ...
 @discussion If the `type` contain `KLImageCacheTypeMemory`, then the `image` will 
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `KLImageCacheTypeDisk`, then the `imageData` will
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(KLImageCacheType)type;
...
/**
 Returns a Boolean value that indicates whether a given key is in cache.
 If the image is not in memory and the `type` contains `KLImageCacheTypeDisk`,
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(KLImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(KLImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, KLImageCacheType type))block;
