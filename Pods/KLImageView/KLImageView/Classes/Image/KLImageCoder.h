...
/**
 Image file type.
 */
typedef NS_ENUM(NSUInteger, KLImageType) {
    KLImageTypeUnknown = 0, ///< unknown
    KLImageTypeJPEG,        ///< jpeg, jpg
    KLImageTypeJPEG2000,    ///< jp2
    KLImageTypeTIFF,        ///< tiff, tif
    KLImageTypeBMP,         ///< bmp
    KLImageTypeICO,         ///< ico
    KLImageTypeICNS,        ///< icns
    KLImageTypeGIF,         ///< gif
    KLImageTypePNG,         ///< png
    KLImageTypeWebP,        ///< webp
    KLImageTypeOther,       ///< other image format
};
 ...
 */
typedef NS_ENUM(NSUInteger, KLImageBlendOperation) {
 ...
 */
@interface KLImageFrame : NSObject <NSCopying>
...
@property (nonatomic) KLImageDisposeMethod dispose;     ///< Frame dispose method.
@property (nonatomic) KLImageBlendOperation blend;      ///< Frame blend operation.
...
/**
 ...
 Example:
    ...
    KLImageDecoder *decoder = [KLImageDecoder decoderWithData:data scale:2.0];
    UIImage image = [decoder frameAtIndex:0 decodeForDisplay:YES].image;
 
    // Decode image during download:
    NSMutableData *data = [NSMutableData new];
    KLImageDecoder *decoder = [[KLImageDecoder alloc] initWithScale:2.0];
 ...
 */
@interface KLImageDecoder : NSObject

@property (nullable, nonatomic, readonly) NSData *data;    ///< Image data.
@property (nonatomic, readonly) KLImageType type;          ///< Image data type.
 ...
 */
- (nullable KLImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay;
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in KLImageType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    KLImageEncoder *jpegEncoder = [[KLImageEncoder alloc] initWithType:KLImageTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    KLImageEncoder *gifEncoder = [[KLImageEncoder alloc] initWithType:KLImageTypeGIF];
 ...
 */
@interface KLImageEncoder : NSObject

@property (nonatomic, readonly) KLImageType type; ///< Image type.
 ...
 */
- (nullable instancetype)initWithType:(KLImageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImage:(UIImage *)image type:(KLImageType)type quality:(CGFloat)quality;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImageWithDecoder:(KLImageDecoder *)decoder type:(KLImageType)type quality:(CGFloat)quality;
...
@interface UIImage (KLImageCoder)
 ...
 */
@property (nonatomic) BOOL kl_isDecodedForDisplay;
 ...
 */
- (void)kl_saveToAlbumWithCompletionBlock:(nullable void(^)(NSURL * _Nullable assetURL, NSError * _Nullable error))completionBlock;
 ...
 */
- (nullable NSData *)kl_imageDataRepresentation;
...
CG_EXTERN KLImageType KLImageDetectType(CFDataRef data);

/// Convert KLImageType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef _Nullable KLImageTypeToUTType(KLImageType type);

/// Convert UTI (such as kUTTypeJPEG) to KLImageType.
CG_EXTERN KLImageType KLImageTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *_Nullable KLImageTypeGetExtension(KLImageType type);



/// Returns the shared DeviceRGB color space.
CG_EXTERN CGColorSpaceRef KLCGColorSpaceGetDeviceRGB();

/// Returns the shared DeviceGray color space.
CG_EXTERN CGColorSpaceRef KLCGColorSpaceGetDeviceGray();

/// Returns whether a color space is DeviceRGB.
CG_EXTERN BOOL KLCGColorSpaceIsDeviceRGB(CGColorSpaceRef space);

/// Returns whether a color space is DeviceGray.
CG_EXTERN BOOL KLCGColorSpaceIsDeviceGray(CGColorSpaceRef space);



/// Convert EXIF orientation value to UIImageOrientation.
CG_EXTERN UIImageOrientation KLUIImageOrientationFromEXIFValue(NSInteger value);

/// Convert UIImageOrientation to EXIF orientation value.
CG_EXTERN NSInteger KLUIImageOrientationToEXIFValue(UIImageOrientation orientation);
 ...
 */
CG_EXTERN CGImageRef _Nullable KLCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay);
 ...
 */
CG_EXTERN CGImageRef _Nullable KLCGImageCreateCopyWithOrientation(CGImageRef imageRef,
...
/**
 ...
 @return A new image, or NULL if an error occurs.
 */
CG_EXTERN CGImageRef _Nullable KLCGImageCreateAffineTransformCopy(CGImageRef imageRef,
 ...
 */
CG_EXTERN CFDataRef _Nullable KLCGImageCreateEncodedData(CGImageRef imageRef, KLImageType type, CGFloat quality);


/**
 Whether WebP is available in KLImage.
 */
CG_EXTERN BOOL KLImageWebPAvailable();
 ...
 */
CG_EXTERN NSUInteger KLImageGetWebPFrameCount(CFDataRef webpData);
 ...
 */
CG_EXTERN CGImageRef _Nullable KLCGImageCreateWithWebPData(CFDataRef webpData,
                                                           BOOL decodeForDisplay,
                                                           BOOL useThreads,
                                                           BOOL bypassFiltering,
                                                           BOOL noFancyUpsampling);

typedef NS_ENUM(NSUInteger, KLImagePreset) {
    KLImagePresetDefault = 0,  ///< default preset.
    KLImagePresetPicture,      ///< digital picture, like portrait, inner shot
    KLImagePresetPhoto,        ///< outdoor photograph, with natural lighting
    KLImagePresetDrawing,      ///< hand or line drawing, with high-contrast details
    KLImagePresetIcon,         ///< small-sized colorful images
    KLImagePresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is KLImagePresetDefault.
 @return WebP data, or nil if an error occurs.
 */
CG_EXTERN CFDataRef _Nullable KLCGImageCreateEncodedWebPData(CGImageRef imageRef,
                                                             BOOL lossless,
                                                             CGFloat quality,
                                                             int compressLevel,
                                                             KLImagePreset preset);
