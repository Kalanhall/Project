...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `KLAnimatedImage` protocol,
 ...
 Sample Code:
     ...
     // ani@3x.gif
     KLImage *image = [KLImage imageNamed:@"ani"];
...
/**
 The KLAnimatedImage protocol declares the required methods for animated image
 display with KLImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to KLImageView.image or KLImageView.highlightedImage
 to display animation.
 
 See `KLImage` and `KLFrameImage` for example.
 */
@protocol KLAnimatedImage <NSObject>
@required
