...
+ (KLImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (KLImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (KLImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        KLImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
        ...
        self.kl_isDecodedForDisplay = YES;
    }
    return self;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"KLImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"KLImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"KLImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"KLImageData"];
    } else {
...
#pragma mark - protocol KLAnimatedImage
