...
static int64_t _KLDeviceMemoryTotal() {
    int64_t mem = [[NSProcessInfo processInfo] physicalMemory];
    if (mem < -1) mem = -1;
        return mem;
}

static int64_t _KLDeviceMemoryFree() {
 ...
 */
@interface _KLImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _KLImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_KLImageWeakProxy alloc] initWithTarget:target];
}
...
typedef NS_ENUM(NSUInteger, KLAnimatedImageType) {
    KLAnimatedImageTypeNone = 0,
    KLAnimatedImageTypeImage,
    KLAnimatedImageTypeHighlightedImage,
    KLAnimatedImageTypeImages,
    KLAnimatedImageTypeHighlightedImages,
};

@interface KLImageView() {
    @package
    UIImage <KLAnimatedImage> *_curAnimatedImage;
...
@interface _KLImageViewFetchOperation : NSOperation
@property (nonatomic, weak) KLImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <KLAnimatedImage> *curImage;
@end

@implementation _KLImageViewFetchOperation
- (void)main {
    __strong KLImageView *view = _view;
    if (!view) return;
...
- (void)resetAnimated {
    if (!_link) {
        ...
        _link = [CADisplayLink displayLinkWithTarget:[_KLImageWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)setImage:(UIImage *)image {
    ...
    [self setImage:image withType:KLAnimatedImageTypeImage];
}

- (void)setHighlightedImage:(UIImage *)highlightedImage {
    if (self.highlightedImage == highlightedImage) return;
    [self setImage:highlightedImage withType:KLAnimatedImageTypeHighlightedImage];
}

- (void)setAnimationImages:(NSArray *)animationImages {
    if (self.animationImages == animationImages) return;
    [self setImage:animationImages withType:KLAnimatedImageTypeImages];
}
...
- (void)setHighlightedAnimationImages:(NSArray *)highlightedAnimationImages {
    ...
    [self setImage:highlightedAnimationImages withType:KLAnimatedImageTypeHighlightedImages];
}
...
- (id)imageForType:(KLAnimatedImageType)type {
    switch (type) {
        case KLAnimatedImageTypeNone: return nil;
        case KLAnimatedImageTypeImage: return self.image;
        case KLAnimatedImageTypeHighlightedImage: return self.highlightedImage;
        case KLAnimatedImageTypeImages: return self.animationImages;
        case KLAnimatedImageTypeHighlightedImages: return self.highlightedAnimationImages;
    }
    return nil;
}

- (KLAnimatedImageType)currentImageType {
    KLAnimatedImageType curType = KLAnimatedImageTypeNone;
    if (self.highlighted) {
        if (self.highlightedAnimationImages.count) curType = KLAnimatedImageTypeHighlightedImages;
        else if (self.highlightedImage) curType = KLAnimatedImageTypeHighlightedImage;
    }
    if (curType == KLAnimatedImageTypeNone) {
        if (self.animationImages.count) curType = KLAnimatedImageTypeImages;
        else if (self.image) curType = KLAnimatedImageTypeImage;
    }
    return curType;
}

- (void)setImage:(id)image withType:(KLAnimatedImageType)type {
    [self stopAnimating];
    if (_link) [self resetAnimated];
    _curFrame = nil;
    switch (type) {
        case KLAnimatedImageTypeNone: break;
        case KLAnimatedImageTypeImage: super.image = image; break;
        case KLAnimatedImageTypeHighlightedImage: super.highlightedImage = image; break;
        case KLAnimatedImageTypeImages: super.animationImages = image; break;
        case KLAnimatedImageTypeHighlightedImages: super.highlightedAnimationImages = image; break;
    }
    [self imageChanged];
}

- (void)imageChanged {
    KLAnimatedImageType newType = [self currentImageType];
    id newVisibleImage = [self imageForType:newType];
    NSUInteger newImageFrameCount = 0;
    BOOL hasContentsRect = NO;
    if ([newVisibleImage isKindOfClass:[UIImage class]] &&
        [newVisibleImage conformsToProtocol:@protocol(KLAnimatedImage)]) {
        newImageFrameCount = ((UIImage<KLAnimatedImage> *) newVisibleImage).animatedImageFrameCount;
        if (newImageFrameCount > 1) {
            hasContentsRect = [((UIImage<KLAnimatedImage> *) newVisibleImage) respondsToSelector:@selector(animatedImageContentsRectAtIndex:)];
        }
    }
    ...
    if (hasContentsRect) {
        CGRect rect = [((UIImage<KLAnimatedImage> *) newVisibleImage) animatedImageContentsRectAtIndex:0];
...
- (void)calcMaxBufferCount {
    ...
    int64_t total = _KLDeviceMemoryTotal();
    int64_t free = _KLDeviceMemoryFree();
...
- (void)startAnimating {
    KLAnimatedImageType type = [self currentImageType];
    if (type == KLAnimatedImageTypeImages || type == KLAnimatedImageTypeHighlightedImages) {
...
- (void)step:(CADisplayLink *)link {
    UIImage <KLAnimatedImage> *image = _curAnimatedImage;
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        _KLImageViewFetchOperation *operation = [_KLImageViewFetchOperation new];
        operation.view = self;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    UIImage *image = [aDecoder decodeObjectForKey:@"KLAnimatedImage"];
    UIImage *highlightedImage = [aDecoder decodeObjectForKey:@"KLHighlightedAnimatedImage"];
    if (image) {
        self.image = image;
        [self setImage:image withType:KLAnimatedImageTypeImage];
    }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    BOOL ani, multi;
    ani = [self.image conformsToProtocol:@protocol(KLAnimatedImage)];
    multi = (ani && ((UIImage <KLAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.image forKey:@"KLAnimatedImage"];
    
    ani = [self.highlightedImage conformsToProtocol:@protocol(KLAnimatedImage)];
    multi = (ani && ((UIImage <KLAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.highlightedImage forKey:@"KLHighlightedAnimatedImage"];
}
