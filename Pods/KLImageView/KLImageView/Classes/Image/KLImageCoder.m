...
#define KL_FOUR_CC(c1,c2,c3,c4) ((uint32_t)(((c4) << 24) | ((c3) << 16) | ((c2) << 8) | (c1)))
#define KL_TWO_CC(c1,c2) ((uint16_t)(((c2) << 8) | (c1)))

static inline uint16_t kl_swap_endian_uint16(uint16_t value) {
    return
    (uint16_t) ((value & 0x00FF) << 8) |
    (uint16_t) ((value & 0xFF00) >> 8) ;
}

static inline uint32_t kl_swap_endian_uint32(uint32_t value) {
    return
...
typedef enum {
    KL_PNG_ALPHA_TYPE_PALEETE = 1 << 0,
    KL_PNG_ALPHA_TYPE_COLOR = 1 << 1,
    KL_PNG_ALPHA_TYPE_ALPHA = 1 << 2,
} kl_png_alpha_type;

typedef enum {
    KL_PNG_DISPOSE_OP_NONE = 0,
    KL_PNG_DISPOSE_OP_BACKGROUND = 1,
    KL_PNG_DISPOSE_OP_PREVIOUS = 2,
} kl_png_dispose_op;

typedef enum {
    KL_PNG_BLEND_OP_SOURCE = 0,
    KL_PNG_BLEND_OP_OVER = 1,
} kl_png_blend_op;
...
typedef struct {
    ...
    uint8_t color_type;         ///< see kl_png_alpha_type
...
typedef struct {
    ...
    uint8_t dispose_op;        ///< see kl_png_dispose_op
    uint8_t blend_op;          ///< see kl_png_blend_op
} kl_png_chunk_fcTL;
...
typedef struct {
    ...
    kl_png_chunk_fcTL frame_control;
} kl_png_frame_info;

typedef struct {
    kl_png_chunk_IHDR header;   ///< png header
    kl_png_chunk_info *chunks;      ///< chunks
    uint32_t chunk_num;          ///< count of chunks
    
    kl_png_frame_info *apng_frames; ///< frame info, NULL if not apng
...
static void kl_png_chunk_IHDR_read(kl_png_chunk_IHDR *IHDR, const uint8_t *data) {
    IHDR->width = kl_swap_endian_uint32(*((uint32_t *)(data)));
    IHDR->height = kl_swap_endian_uint32(*((uint32_t *)(data + 4)));
...
static void kl_png_chunk_IHDR_write(kl_png_chunk_IHDR *IHDR, uint8_t *data) {
    *((uint32_t *)(data)) = kl_swap_endian_uint32(IHDR->width);
    *((uint32_t *)(data + 4)) = kl_swap_endian_uint32(IHDR->height);
...
static void kl_png_chunk_fcTL_read(kl_png_chunk_fcTL *fcTL, const uint8_t *data) {
    fcTL->sequence_number = kl_swap_endian_uint32(*((uint32_t *)(data)));
    fcTL->width = kl_swap_endian_uint32(*((uint32_t *)(data + 4)));
    fcTL->height = kl_swap_endian_uint32(*((uint32_t *)(data + 8)));
    fcTL->x_offset = kl_swap_endian_uint32(*((uint32_t *)(data + 12)));
    fcTL->y_offset = kl_swap_endian_uint32(*((uint32_t *)(data + 16)));
    fcTL->delay_num = kl_swap_endian_uint16(*((uint16_t *)(data + 20)));
    fcTL->delay_den = kl_swap_endian_uint16(*((uint16_t *)(data + 22)));
    fcTL->dispose_op = data[24];
    fcTL->blend_op = data[25];
}

static void kl_png_chunk_fcTL_write(kl_png_chunk_fcTL *fcTL, uint8_t *data) {
    *((uint32_t *)(data)) = kl_swap_endian_uint32(fcTL->sequence_number);
    *((uint32_t *)(data + 4)) = kl_swap_endian_uint32(fcTL->width);
    *((uint32_t *)(data + 8)) = kl_swap_endian_uint32(fcTL->height);
    *((uint32_t *)(data + 12)) = kl_swap_endian_uint32(fcTL->x_offset);
    *((uint32_t *)(data + 16)) = kl_swap_endian_uint32(fcTL->y_offset);
    *((uint16_t *)(data + 20)) = kl_swap_endian_uint16(fcTL->delay_num);
    *((uint16_t *)(data + 22)) = kl_swap_endian_uint16(fcTL->delay_den);
    data[24] = fcTL->dispose_op;
    data[25] = fcTL->blend_op;
}

// convert double value to fraction
static void kl_png_delay_to_fraction(double duration, uint16_t *num, uint16_t *den) {
    if (duration >= 0xFF) {
        *num = 0xFF;
        *den = 1;
...
static double kl_png_delay_to_seconds(uint16_t num, uint16_t den) {
    if (den == 0) {
        return num / 100.0;
    } else {
...
static bool kl_png_validate_animation_chunk_order(kl_png_chunk_info *chunks,  /* input */
    ...
    if (chunks->fourcc != KL_FOUR_CC('I', 'H', 'D', 'R')) return false;
    if ((chunks + chunk_num - 1)->fourcc != KL_FOUR_CC('I', 'E', 'N', 'D')) return false;
    ...
    for (uint32_t i = 0; i < chunk_num; i++) {
        kl_png_chunk_info *chunk = chunks + i;
        switch (chunk->fourcc) {
            case KL_FOUR_CC('I', 'H', 'D', 'R'): {  // png header
                if (i != 0) return false;
                if (IHDR_num > 0) return false;
                IHDR_num++;
            } break;
            case KL_FOUR_CC('I', 'D', 'A', 'T'): {  // png data
                if (prev_fourcc != KL_FOUR_CC('I', 'D', 'A', 'T')) {
                    if (IDAT_num == 0)
                        first_IDAT = i;
                    else
                        return false;
            ...
            case KL_FOUR_CC('a', 'c', 'T', 'L'): {  // apng control
                if (acTL_num > 0) return false;
                acTL_num++;
            } break;
            case KL_FOUR_CC('f', 'c', 'T', 'L'): {  // apng frame control
                if (i + 1 == chunk_num) return false;
                if ((chunk + 1)->fourcc != KL_FOUR_CC('f', 'd', 'A', 'T') &&
                    (chunk + 1)->fourcc != KL_FOUR_CC('I', 'D', 'A', 'T')) {
                    return false;
                }
                if (fcTL_num == 0) {
                    if ((chunk + 1)->fourcc == KL_FOUR_CC('I', 'D', 'A', 'T')) {
                        first_frame_cover = true;
                    }
                }
                fcTL_num++;
            } break;
            case KL_FOUR_CC('f', 'd', 'A', 'T'): {  // apng data
                if (prev_fourcc != KL_FOUR_CC('f', 'd', 'A', 'T') && prev_fourcc != KL_FOUR_CC('f', 'c', 'T', 'L')) {
                    return false;
                }
            } break;
        }
...
static void kl_png_info_release(kl_png_info *info) {
    if (info) {
...
/**
 ...
 @return A png info object, you may call kl_png_info_release() to release it.
 Returns NULL if an error occurs.
 */
static kl_png_info *kl_png_info_create(const uint8_t *data, uint32_t length) {
    if (length < 32) return NULL;
    if (*((uint32_t *)data) != KL_FOUR_CC(0x89, 0x50, 0x4E, 0x47)) return NULL;
    if (*((uint32_t *)(data + 4)) != KL_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A)) return NULL;
    
    uint32_t chunk_realloc_num = 16;
    kl_png_chunk_info *chunks = malloc(sizeof(kl_png_chunk_info) * chunk_realloc_num);
    ...
    do {
        if (chunk_num >= chunk_capacity) {
            kl_png_chunk_info *new_chunks = realloc(chunks, sizeof(kl_png_chunk_info) * (chunk_capacity + chunk_realloc_num));
            if (!new_chunks) {
                free(chunks);
                return NULL;
            }
        ...
        kl_png_chunk_info *chunk = chunks + chunk_num;
        const uint8_t *chunk_data = data + offset;
        chunk->offset = offset;
        chunk->length = kl_swap_endian_uint32(*((uint32_t *)chunk_data));
        ...
        chunk->crc32 = kl_swap_endian_uint32(*((uint32_t *)(chunk_data + 8 + chunk->length)));
        chunk_num++;
        offset += 12 + chunk->length;
        
        switch (chunk->fourcc) {
            case KL_FOUR_CC('a', 'c', 'T', 'L') : {
                if (chunk->length == 8) {
                    apng_frame_number = kl_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
                    apng_loop_num = kl_swap_endian_uint32(*((uint32_t *)(chunk_data + 12)));
                } else {
                    apng_chunk_error = true;
                }
            } break;
            case KL_FOUR_CC('f', 'c', 'T', 'L') :
            case KL_FOUR_CC('f', 'd', 'A', 'T') : {
                if (chunk->fourcc == KL_FOUR_CC('f', 'c', 'T', 'L')) {
                    if (chunk->length != 26) {
                        apng_chunk_error = true;
                    } else {
                        apng_frame_index++;
                ...
                if (chunk->length > 4) {
                    uint32_t sequence = kl_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
    ...
    if (chunk_num < 3 ||
        chunks->fourcc != KL_FOUR_CC('I', 'H', 'D', 'R') ||
        chunks->length != 13) {
        free(chunks);
        return NULL;
    }
    ...
    // png info
    kl_png_info *info = calloc(1, sizeof(kl_png_info));
    if (!info) {
        free(chunks);
        return NULL;
    }
    ...
    kl_png_chunk_IHDR_read(&info->header, data + chunks->offset + 8);
    ...
    if (!apng_chunk_error && apng_frame_number == apng_frame_index && apng_frame_number >= 1) {
        ...
        if (!kl_png_validate_animation_chunk_order(info->chunks, info->chunk_num, &first_IDAT_index, &first_frame_is_cover)) {
        ...
        info->apng_frames = calloc(apng_frame_number, sizeof(kl_png_frame_info));
        if (!info->apng_frames) {
            kl_png_info_release(info);
            return NULL;
        }
        ...
        for (int32_t i = 0; i < info->chunk_num; i++) {
            kl_png_chunk_info *chunk = info->chunks + i;
            switch (chunk->fourcc) {
                case KL_FOUR_CC('I', 'D', 'A', 'T'): {
                    if (info->apng_shared_insert_index == 0) {
                        info->apng_shared_insert_index = i;
                    }
                    if (first_frame_is_cover) {
                        kl_png_frame_info *frame = info->apng_frames + frame_index;
                        frame->chunk_num++;
                ...
                case KL_FOUR_CC('f', 'c', 'T', 'L'): {
                    ...
                    kl_png_chunk_fcTL_read(&frame->frame_control, data + chunk->offset + 8);
                } break;
                case KL_FOUR_CC('f', 'd', 'A', 'T'): {
                    kl_png_frame_info *frame = info->apng_frames + frame_index;
                    frame->chunk_num++;
...
/**
 ...
 Returns NULL if an error occurs.
 */
static uint8_t *kl_png_copy_frame_data_at_index(const uint8_t *data,
                                                const kl_png_info *info,
                                                const uint32_t index,
                                                uint32_t *size) {
    if (index >= info->apng_frame_num) return NULL;
    
    kl_png_frame_info *frame_info = info->apng_frames + index;
    ...
    *size = frame_remux_size;
    ...
    for (uint32_t i = 0; i < info->apng_shared_chunk_num; i++) {
        ...
        kl_png_chunk_info *shared_chunk_info = info->chunks + shared_chunk_index;
        ...
        if (shared_chunk_index >= info->apng_shared_insert_index && !inserted) { // replace IDAT with fdAT
            ...
            for (uint32_t c = 0; c < frame_info->chunk_num; c++) {
                kl_png_chunk_info *insert_chunk_info = info->chunks + frame_info->chunk_index + c;
                if (insert_chunk_info->fourcc == KL_FOUR_CC('f', 'd', 'A', 'T')) {
                    *((uint32_t *)(frame_data + data_offset)) = kl_swap_endian_uint32(insert_chunk_info->length - 4);
                    *((uint32_t *)(frame_data + data_offset + 4)) = KL_FOUR_CC('I', 'D', 'A', 'T');
                    ...
                    *((uint32_t *)(frame_data + data_offset + insert_chunk_info->length + 4)) = kl_swap_endian_uint32(crc);
        ...
        if (shared_chunk_info->fourcc == KL_FOUR_CC('I', 'H', 'D', 'R')) {
            uint8_t tmp[25] = {0};
            memcpy(tmp, data + shared_chunk_info->offset, 25);
            kl_png_chunk_IHDR IHDR = info->header;
            IHDR.width = frame_info->frame_control.width;
            IHDR.height = frame_info->frame_control.height;
            kl_png_chunk_IHDR_write(&IHDR, tmp + 8);
            *((uint32_t *)(tmp + 21)) = kl_swap_endian_uint32((uint32_t)crc32(0, tmp + 4, 17));
...
static inline size_t KLImageByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

/// Convert degree to radians
static inline CGFloat KLImageDegreesToRadians(CGFloat degrees) {
    return degrees * M_PI / 180;
}

CGColorSpaceRef KLCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef KLCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL KLCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, KLCGColorSpaceGetDeviceRGB());
}

BOOL KLCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, KLCGColorSpaceGetDeviceGray());
}
...
/**
 ...
 CGDataProviderRef provider = CGDataProviderCreateWithData(data, data, size, KLCGDataProviderReleaseDataCallback);
 */
static void KLCGDataProviderReleaseDataCallback(void *info, const void *data, size_t size) {
    if (info) free(info);
}
 ...
 */
static BOOL KLCGImageDecodeToBitmapBufferWithAnyFormat(CGImageRef srcImage, vImage_Buffer *dest, vImage_CGImageFormat *destFormat) {
...
/**
 ...
 @return Whether succeed.
 */
static BOOL KLCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = KLCGColorSpaceGetDeviceRGB();
    destFormat.bitmapInfo = bitmapInfo;
    dest->data = NULL;
    if (KLCGImageDecodeToBitmapBufferWithAnyFormat(srcImage, dest, &destFormat)) return YES;
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, KLCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef KLCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, KLCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef KLCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    if(!KLCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &src, kCGImageAlphaFirst | kCGBitmapByteOrderDefault)) return NULL;
    
    size_t destBytesPerRow = KLImageByteAlign(destWidth * 4, 32);
    ...
    tmpProvider = CGDataProviderCreateWithData(tmp.data, tmp.data, destHeight * destBytesPerRow, KLCGDataProviderReleaseDataCallback);
    if (!tmpProvider) goto fail;
    tmp.data = NULL; // hold by provider
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, KLCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    if (!KLCGImageDecodeToBitmapBufferWith32BitFormat(tmpImage, &dest, destBitmapInfo)) goto fail;
    CFRelease(tmpImage);
    tmpImage = NULL;
    
    destProvider = CGDataProviderCreateWithData(dest.data, dest.data, destHeight * destBytesPerRow, KLCGDataProviderReleaseDataCallback);
    if (!destProvider) goto fail;
    dest.data = NULL; // hold by provider
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, KLCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation KLUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger KLUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef KLCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    switch (orientation) {
        case UIImageOrientationDown: {
            transform = CGAffineTransformMakeRotation(KLImageDegreesToRadians(180));
            transform = CGAffineTransformTranslate(transform, -(CGFloat)width, -(CGFloat)height);
        } break;
        case UIImageOrientationLeft: {
            transform = CGAffineTransformMakeRotation(KLImageDegreesToRadians(90));
    ...
    return KLCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

KLImageType KLImageDetectType(CFDataRef data) {
    if (!data) return KLImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return KLImageTypeUnknown;
    
    const char *bytes = (char *)CFDataGetBytePtr(data);
    
    uint32_t magic4 = *((uint32_t *)bytes);
    switch (magic4) {
        case KL_FOUR_CC(0x4D, 0x4D, 0x00, 0x2A): { // big endian TIFF
            return KLImageTypeTIFF;
        } break;
            
        case KL_FOUR_CC(0x49, 0x49, 0x2A, 0x00): { // little endian TIFF
            return KLImageTypeTIFF;
        } break;
            
        case KL_FOUR_CC(0x00, 0x00, 0x01, 0x00): { // ICO
            return KLImageTypeICO;
        } break;
            
        case KL_FOUR_CC(0x00, 0x00, 0x02, 0x00): { // CUR
            return KLImageTypeICO;
        } break;
            
        case KL_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return KLImageTypeICNS;
        } break;
            
        case KL_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return KLImageTypeGIF;
        } break;
            
        case KL_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == KL_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return KLImageTypePNG;
            }
        } break;
            
        case KL_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            uint32_t tmp = *((uint32_t *)(bytes + 8));
            if (tmp == KL_FOUR_CC('W', 'E', 'B', 'P')) {
                return KLImageTypeWebP;
            }
        } break;
        /*
        case KL_FOUR_CC('B', 'P', 'G', 0xFB): { // BPG
            return KLImageTypeBPG;
        } break;
        */
    }
    ...
    switch (magic2) {
        ...
        case KL_TWO_CC('P', 'I'):
        case KL_TWO_CC('C', 'I'):
        case KL_TWO_CC('C', 'P'): { // BMP
            return KLImageTypeBMP;
        }
        case KL_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return KLImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return KLImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return KLImageTypeJPEG2000;
    
    return KLImageTypeUnknown;
}

CFStringRef KLImageTypeToUTType(KLImageType type) {
    switch (type) {
        case KLImageTypeJPEG: return kUTTypeJPEG;
        case KLImageTypeJPEG2000: return kUTTypeJPEG2000;
        case KLImageTypeTIFF: return kUTTypeTIFF;
        case KLImageTypeBMP: return kUTTypeBMP;
        case KLImageTypeICO: return kUTTypeICO;
        case KLImageTypeICNS: return kUTTypeAppleICNS;
        case KLImageTypeGIF: return kUTTypeGIF;
        case KLImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

KLImageType KLImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(KLImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(KLImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(KLImageTypeTIFF),
                (id)kUTTypeBMP : @(KLImageTypeBMP),
                (id)kUTTypeICO : @(KLImageTypeICO),
                (id)kUTTypeAppleICNS : @(KLImageTypeICNS),
                (id)kUTTypeGIF : @(KLImageTypeGIF),
                (id)kUTTypePNG : @(KLImageTypePNG)};
    });
    if (!uti) return KLImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *KLImageTypeGetExtension(KLImageType type) {
    switch (type) {
        case KLImageTypeJPEG: return @"jpg";
        case KLImageTypeJPEG2000: return @"jp2";
        case KLImageTypeTIFF: return @"tiff";
        case KLImageTypeBMP: return @"bmp";
        case KLImageTypeICO: return @"ico";
        case KLImageTypeICNS: return @"icns";
        case KLImageTypeGIF: return @"gif";
        case KLImageTypePNG: return @"png";
        case KLImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef KLCGImageCreateEncodedData(CGImageRef imageRef, KLImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == KLImageTypeWebP) {
#if KLIMAGE_WEBP_ENABLED
        if (quality == 1) {
            return KLCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, KLImagePresetDefault);
        } else {
            return KLCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, KLImagePresetDefault);
        }
#else
        return NULL;
#endif
    ...
    CFStringRef uti = KLImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL KLImageWebPAvailable() {
    return YES;
}

CFDataRef KLCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, KLImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    if(!KLCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &buffer, kCGImageAlphaLast | kCGBitmapByteOrderDefault)) return nil;
    ...
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    preset = preset > KLImagePresetText ? KLImagePresetDefault : preset;
...
NSUInteger KLImageGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef KLCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    bitsPerComponent = 8;
    bitsPerPixel = 32;
    bytesPerRow = KLImageByteAlign(bitsPerPixel / 8 * canvasWidth, 32);
    ...
    provider = CGDataProviderCreateWithData(destBytes, destBytes, destLength, KLCGDataProviderReleaseDataCallback);
    if (!provider) goto fail;
    destBytes = NULL; // hold by provider
    
    imageRef = CGImageCreate(canvasWidth, canvasHeight, bitsPerComponent, bitsPerPixel, bytesPerRow, KLCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
@implementation KLImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    KLImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    KLImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _KLImageDecoderFrame : KLImageFrame
...
@implementation _KLImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    _KLImageDecoderFrame *frame = [super copyWithZone:zone];
...
- (void)dealloc {
    ...
    if (_apngSource) kl_png_info_release(_apngSource);
#if KLIMAGE_WEBP_ENABLED
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    KLImageDecoder *decoder = [[KLImageDecoder alloc] initWithScale:scale];
...
- (KLImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    KLImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((_KLImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    KLImageType type = KLImageDetectType((__bridge CFDataRef)data);
...
- (KLImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _KLImageDecoderFrame *frame = [(_KLImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != KLImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = KLCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
        ...
        CFRelease(imageRef);
        if (!image) return nil;
        image.kl_isDecodedForDisplay = decoded;
        frame.image = image;
        return frame;
    }
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == KLImageDisposeBackground) {
    ...
    image.kl_isDecodedForDisplay = YES;
    frame.image = image;
    if (extendToCanvas) {
        frame.width = _width;
        frame.height = _height;
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = KLImageDisposeNone;
        frame.blend = KLImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSourceWebP {
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            _KLImageDecoderFrame *frame = [_KLImageDecoderFrame new];
            [frames addObject:frame];
            if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
                frame.dispose = KLImageDisposeBackground;
            }
            if (iter.blend_method == WEBP_MUX_BLEND) {
                frame.blend = KLImageBlendOver;
            }
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/kalan/KLImage#installation", func, line);
    });
#endif
}
...
- (void)_updateSourceAPNG {
    ...
    kl_png_info_release(_apngSource);
    _apngSource = nil;
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        kl_png_frame_info *fi = apng->apng_frames + i;
        frame.index = i;
        frame.duration = kl_png_delay_to_seconds(fi->frame_control.delay_num, fi->frame_control.delay_den);
        frame.hasAlpha = YES;
        ...
        switch (fi->frame_control.dispose_op) {
            case KL_PNG_DISPOSE_OP_BACKGROUND: {
                frame.dispose = KLImageDisposeBackground;
            } break;
            case KL_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = KLImageDisposePrevious;
            } break;
            default: {
                frame.dispose = KLImageDisposeNone;
            } break;
        ...
        if (frame.blend == KLImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != KLImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == KLImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == KLImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == KLImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = KLUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    _KLImageDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = KLCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, KLCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        uint32_t size = 0;
        uint8_t *bytes = kl_png_copy_frame_data_at_index(_data.bytes, _apngSource, (uint32_t)index, &size);
        if (!bytes) return NULL;
        CGDataProviderRef provider = CGDataProviderCreateWithData(bytes, bytes, size, KLCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(bytes);
            return NULL;
        }
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, KLCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        size_t bytesPerRow = KLImageByteAlign(bitsPerPixel / 8 * width, 32);
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(pixels, pixels, length, KLCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(pixels);
            return NULL;
        }
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, KLCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, KLCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(_KLImageDecoderFrame *)frame {
    if (frame.dispose == KLImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == KLImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == KLImageBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(_KLImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == KLImageDisposePrevious) {
        if (frame.blend == KLImageBlendOver) {
...
@implementation KLImageEncoder {
    NSMutableArray *_images;
    NSMutableArray *_durations;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"KLImageEncoder init error" reason:@"KLImageEncoder must be initialized with a type. Use 'initWithType:' instead." userInfo:nil];
    return [self initWithType:KLImageTypeUnknown];
}

- (instancetype)initWithType:(KLImageType)type {
    if (type == KLImageTypeUnknown || type >= KLImageTypeOther) {
...
#if !KLIMAGE_WEBP_ENABLED
    if (type == KLImageTypeWebP) {
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/kalan/KLImage#installation", __FUNCTION__, __LINE__);
        return nil;
    }
#endif
    ...
    switch (type) {
        case KLImageTypeJPEG:
        case KLImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case KLImageTypeTIFF:
        case KLImageTypeBMP:
        case KLImageTypeGIF:
        case KLImageTypeICO:
        case KLImageTypeICNS:
        case KLImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, KLImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, KLImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == KLImageTypeGIF && count > 1) {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = KLCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return KLCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return KLCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == KLImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = KLCGImageCreateEncodedData(decoded, KLImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, KLCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = KLCGImageCreateEncodedData(extendedImage, KLImageTypePNG, 1);
        if (!frameData) {
    ...
    kl_png_info *info = kl_png_info_create(firstFrameData.bytes, (uint32_t)firstFrameData.length);
    if (!info) return nil;
    ...
    png_header[0] = KL_FOUR_CC(0x89, 0x50, 0x4E, 0x47);
    png_header[1] = KL_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A);
    ...
    for (int i = 0; i < info->chunk_num; i++) {
        ...
        if (!insertBefore && chunk->fourcc == KL_FOUR_CC('I', 'D', 'A', 'T')) {
            insertBefore = YES;
            // insert acTL (APNG Control)
            uint32_t acTL[5] = {0};
            acTL[0] = kl_swap_endian_uint32(8); //length
            acTL[1] = KL_FOUR_CC('a', 'c', 'T', 'L'); // fourcc
            acTL[2] = kl_swap_endian_uint32((uint32_t)pngDatas.count); // num frames
            acTL[3] = kl_swap_endian_uint32((uint32_t)_loopCount); // num plays
            acTL[4] = kl_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(acTL + 1), 12)); //crc32
            [result appendBytes:acTL length:20];
            
            // insert fcTL (first frame control)
            kl_png_chunk_fcTL chunk_fcTL = {0};
            ...
            kl_png_delay_to_fraction([(NSNumber *)_durations[0] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
            chunk_fcTL.delay_num = chunk_fcTL.delay_num;
            chunk_fcTL.delay_den = chunk_fcTL.delay_den;
            chunk_fcTL.dispose_op = KL_PNG_DISPOSE_OP_BACKGROUND;
            chunk_fcTL.blend_op = KL_PNG_BLEND_OP_SOURCE;
            
            uint8_t fcTL[38] = {0};
            *((uint32_t *)fcTL) = kl_swap_endian_uint32(26); //length
            *((uint32_t *)(fcTL + 4)) = KL_FOUR_CC('f', 'c', 'T', 'L'); // fourcc
            kl_png_chunk_fcTL_write(&chunk_fcTL, fcTL + 8);
            *((uint32_t *)(fcTL + 34)) = kl_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
            [result appendBytes:fcTL length:38];
            
            apngSequenceIndex++;
        }
        
        if (!insertAfter && insertBefore && chunk->fourcc != KL_FOUR_CC('I', 'D', 'A', 'T')) {
            insertAfter = YES;
            // insert fcTL and fdAT (APNG frame control and data)
            
            for (int i = 1; i < pngDatas.count; i++) {
                NSData *frameData = pngDatas[i];
                kl_png_info *frame = kl_png_info_create(frameData.bytes, (uint32_t)frameData.length);
                if (!frame) {
                    kl_png_info_release(info);
                    return nil;
                }
                ...
                kl_png_delay_to_fraction([(NSNumber *)_durations[i] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
                chunk_fcTL.delay_num = chunk_fcTL.delay_num;
                chunk_fcTL.delay_den = chunk_fcTL.delay_den;
                chunk_fcTL.dispose_op = KL_PNG_DISPOSE_OP_BACKGROUND;
                chunk_fcTL.blend_op = KL_PNG_BLEND_OP_SOURCE;
                ...
                *((uint32_t *)(fcTL + 34)) = kl_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
                ...
                for (int d = 0; d < frame->chunk_num; d++) {
                    kl_png_chunk_info *dchunk = frame->chunks + d;
                    if (dchunk->fourcc == KL_FOUR_CC('I', 'D', 'A', 'T')) {
                        uint32_t length = kl_swap_endian_uint32(dchunk->length + 4);
                        [result appendBytes:&length length:4]; //length
                        uint32_t fourcc = KL_FOUR_CC('f', 'd', 'A', 'T');
                        [result appendBytes:&fourcc length:4]; //fourcc
                        uint32_t sq = kl_swap_endian_uint32(apngSequenceIndex);
                        ...
                        uint32_t crc = kl_swap_endian_uint32((uint32_t)crc32(0, bytes, dchunk->length + 8));
                        [result appendBytes:&crc length:4]; //crc
                        
                        apngSequenceIndex++;
                    }
                }
                kl_png_info_release(frame);
            }
        }
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = KLCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, KLImagePresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
- (NSData *)encode {
    ...
    if (_type == KLImageTypePNG) return [self _encodeAPNG];
    if (_type == KLImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(KLImageType)type quality:(CGFloat)quality {
    KLImageEncoder *encoder = [[KLImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(KLImageDecoder *)decoder type:(KLImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (KLImageCoder)

- (instancetype)kl_imageByDecoded {
    if (self.kl_isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = KLCGImageCreateDecodedCopy(imageRef, YES);
    ...
    newImage.kl_isDecodedForDisplay = YES;
    return newImage;
}

- (BOOL)kl_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[KLSpriteSheetImage class]]) return YES;
    NSNumber *num = objc_getAssociatedObject(self, @selector(kl_isDecodedForDisplay));
    return [num boolValue];
}

- (void)setKl_isDecodedForDisplay:(BOOL)isDecodedForDisplay {
    objc_setAssociatedObject(self, @selector(kl_isDecodedForDisplay), @(isDecodedForDisplay), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)kl_saveToAlbumWithCompletionBlock:(void(^)(NSURL *assetURL, NSError *error))completionBlock {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSData *data = [self _kl_dataRepresentationForSystem:YES];
...
- (NSData *)kl_imageDataRepresentation {
    return [self _kl_dataRepresentationForSystem:NO];
}

/// @param forSystem YES: used for system album (PNG/JPEG/GIF), NO: used for KLImage (PNG/JPEG/GIF/WebP)
- (NSData *)_kl_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[KLImage class]]) {
        KLImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == KLImageTypeGIF ||
                    image.animatedImageType == KLImageTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = KLCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
