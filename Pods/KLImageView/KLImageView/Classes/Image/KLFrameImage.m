...
- (instancetype)initWithImagePaths:(NSArray *)paths frameDurations:(NSArray *)frameDurations loopCount:(NSUInteger)loopCount {
    ...
    UIImage *firstCG = [[[UIImage alloc] initWithData:firstData] kl_imageByDecoded];
...
- (UIImage *)animatedImageFrameAtIndex:(NSUInteger)index {
    if (_imagePaths) {
        ...
        return [[UIImage imageWithData:data scale:scale] kl_imageByDecoded];
    } else if (_imageDatas) {
        if (index >= _imageDatas.count) return nil;
        NSData *data = _imageDatas[index];
        return [[UIImage imageWithData:data scale:[UIScreen mainScreen].scale] kl_imageByDecoded];
    } else {
